        ...
        if (existingEntry) {
          ...
          if (!existingEntry.completedOnce) {
            if (isCorrect) {
              ...
              const ex = req.body.exercise.split("-");
              if (req.body.exercise.split("-")[1] === "topic") {
                user.certificates.push(
                  "Cody Certified: " + ex[0] + " " + ex.pop() + " User"
                );
              } else {
                let lessonsCompleted = 0;
                user.qAnswers.forEach((ans) => {
                  if (ans.exercise === req.body.exercise) {
                    lessonsCompleted += 1;
                  }
                });
                if (data.maxLessons === lessonsCompleted) {
                  ex =
          ...
          let data = {
            ...
            maxLessons: req.body.maxLessons,
          };

          if (isCorrect) {
            let ex = req.body.exercise.split("-");
            if (data.isCourse === false) {
              ex = "Cody Certified: " + ex[0] + " " + ex.pop() + " User";
              user.certificates.push(ex);
            } else {
              let lessonsCompleted = 0;
              user.qAnswers.forEach((ans) => {
              ...
              if (data.maxLessons === lessonsCompleted) {
                ex = "Cody Course Mastery Certified: " + ex[0] + " " + ex.pop();
                user.certificates.push(ex);
              } else {
                console("HERE");
                data.completedOnce = false;
